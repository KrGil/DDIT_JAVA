...
public class StringClass {
	...
	public static void main(String[] args) {
		 ...
		 *  - substring() : 문자열의 특정 부분을 잘라서 반환하다.
		...
		String str1 = "문자열";
		String str2 = null;	//.으로 파고 들어 equals 메서드를 찾지만 null에는 equals란 메서드가 존재하지 x
		System.out.println(str1.equals(str2));
//		System.out.println(str2.equals(str1));//따라서 변수가 null인 애들은 앞에 처리하지 않는다. 변할거 같은 값은 뒤로!
		// null이 있을 수 있는 변수를 앞에 두지 않는다.
		
		String str = "12345";
		int length = str.length(); // int타입으로 알려준다.
		...
		for(int i = 0; i < menu.length; i ++){
			...
			int number = Integer.parseInt(menu[i].substring(menu[i].indexOf(" ")+1,
					menu[i].indexOf("원")));
